...
t_bool	is_cylinder_bottom(t_cy cylinder, t_ray main_ray, t_vec *normal_cy)
{
	...
	dist_center = __sq(main_ray.o.x + main_ray.len * main_ray.dir.x - \
			(cylinder.o.x + cylinder.h / 2 * (-1) * cylinder.axis.x));
	dist_center += __sq(main_ray.o.y + main_ray.len * main_ray.dir.y - \
			(cylinder.o.y + cylinder.h / 2 * (-1) * cylinder.axis.y));
	dist_center += __sq(main_ray.o.z + main_ray.len * main_ray.dir.z - \
			(cylinder.o.z + cylinder.h / 2 * (-1) * cylinder.axis.z));
...
t_bool	is_cylinder_top(t_cy cylinder, t_ray main_ray, t_vec *normal_cy)
{
	...
	dist_top_center = __sq(main_ray.o.x + main_ray.len * main_ray.dir.x - \
				(cylinder.o.x + cylinder.h / 2 * cylinder.axis.x));
	dist_top_center += __sq(main_ray.o.y + main_ray.len * main_ray.dir.y - \
				(cylinder.o.y + cylinder.h / 2 * cylinder.axis.y));
	dist_top_center += __sq(main_ray.o.z + main_ray.len * main_ray.dir.z - \
				(cylinder.o.z + cylinder.h / 2 * cylinder.axis.z));
...
t_bool	cylinder_lightning(t_vars *v, t_ray light_ray, int i, int j)
{
	...
	{
		color = amb_color(*v->amb, \
			compute_color(&v->cy[v->ray->obj.index]->rgb));
