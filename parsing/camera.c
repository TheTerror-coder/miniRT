...
/*   Updated: 2024/01/09 19:52:05 by lmohin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	pov_parsing(char **pov, size_t line_index);
t_bool	set_cam_dir(t_vars *v, char *infodir, size_t line_index);
t_bool	cam_dir_parsing(char **dir, size_t line_index);
...
t_bool	set_cam_dir(t_vars *v, char *infodir, size_t line_index)
{
	...
	if (!cam_dir_parsing(dir, line_index))
	{
		ft_free2str(&dir);
		return (__FALSE);
	}
	...
	ft_free2str(&dir);
	if (v->cam->dir.x * v->cam->dir.x + v->cam->dir.y * v->cam->dir.y
		+ v->cam->dir.z * v->cam->dir.z != 1)
